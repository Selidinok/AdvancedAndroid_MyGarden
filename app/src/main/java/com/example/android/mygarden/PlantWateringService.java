...
public class PlantWateringService extends IntentService {
    ...
    private void handleActionUpdatePlantWidgets() {
        Cursor cursor = getContentResolver().query(PlantContract.PlantEntry.CONTENT_URI,
                null,
                null,
                null,
                PlantContract.PlantEntry.COLUMN_LAST_WATERED_TIME);
        ...
        if (cursor != null && cursor.getCount() > 0){
            ...
            int lastWateredTimeIndex = cursor.getColumnIndex(PlantContract.PlantEntry.COLUMN_LAST_WATERED_TIME);
            long timeNow = System.currentTimeMillis();
            int plantType = cursor.getInt(plantTypeIndex);
            long creationTime = cursor.getLong(creationTimeIndex);
            long lastWateredTime = cursor.getLong(lastWateredTimeIndex);
            cursor.close();

            imgRes = PlantUtils.getPlantImageRes(this,
                    timeNow - creationTime,
                    timeNow - lastWateredTime,
                    plantType);
        }
